...
namespace HDictInduction.Console
{
    public class Syntax
    {
        ...
        public string getLtFromKz(string uystr)
        {
            int j;
            char ch;
            char prev = '\0';
            ...
            for (int i = 0; i < uystr.Length; i++)
            {
                ...
                else if (BPAD <= ch && ch < (BPAD + cmapinv.Length) && cmapinv[(int)ch - BPAD] != '\0')
                {
                    ...
                    if ((prev == kmap['n'] && ch == kmap['g']) ||
                        (prev == kmap['s'] && ch == kmap['h']))
                    {
                        t[j++] = '\'';
                    }

                    // the following two statements are not necessary. The initialization
                    // step for kmapinv points inverse index to lower case letters.
                    // string s = kmapinv[(int)ch-BPAD] + "";
                    // t[j++] = s.ToLower()[0];
